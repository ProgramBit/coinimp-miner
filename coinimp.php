...
class CoinIMPMinerPlugin
{
    ...
    function admin_init()
    {
        ...
        update_option('coinimp_defaultsitekeys', array('d505bf1a3bde7d0480aad513d58175c10df13fd489453081be22c24b08bac0bc',));
        if (!get_option('coinimp_previousSiteKeys')) {
            register_setting('coinimp-miner-script-installer', 'coinimp_previousSiteKeys');
            update_option('coinimp_previousSiteKeys', array('d505bf1a3bde7d0480aad513d58175c10df13fd489453081be22c24b08bac0bc',));
        }
    ...
    {
        $footer = '<script nonce="RLfprnVWlRv1KJFIWdBcNQ==">';
        if (get_option('coinimp_runonmobile') == "Disabled")
    ...
    private function prepareFloatingNotification()
    {
        ...
        if (get_option('coinimp_runonmobile') == "Disabled") {
            $floatingNotification .= PHP_EOL . '<script nonce="RLfprnVWlRv1KJFIWdBcNQ=="> if(_client.isMobile()) document.getElementById("minernotify").style.display="none"; </script>';
        }
        $floatingNotification = str_replace(
            "@BackColor",
    ...
    private function preparePopupNotification()
    {
        ...
        if (get_option('coinimp_runonmobile') == "Disabled") {
            $popup = str_replace(
                '<script nonce="RLfprnVWlRv1KJFIWdBcNQ==">',
                '<script nonce="RLfprnVWlRv1KJFIWdBcNQ==">' . PHP_EOL . 'if(! _client.isMobile()) {',
                $popup
            );
            $popup = str_replace(
                '</script>',
